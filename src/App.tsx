...
export default function App() {
  ...
  const getAI = () => {
    ...
    if (!key) {
      throw new Error(
        "GEMINI_API_KEY missing. Please add it to your environment variables.",
      );
    }
  ...
  const startRecording = async () => {
    ...
    try {
      ...
      try {
        if (sourceType === "screen") {
          ...
          try {
            ...
            console.warn(
              "Screen capture with audio failed, trying video only:",
              ve,
            );
            try {
              ...
              if (
                ve2.name === "NotAllowedError" ||
                ve2.message.includes("permissions policy")
              ) {
                throw new Error(
                  "ACCESS BLOCKED: Click 'Open in new tab' at the top right to use Screen Feed.",
                );
              }
              throw new Error(
                "Screen capture failed. Check if you closed the selector or denied permissions.",
              );
            }
          }
          ...
          if (captureStream.getAudioTracks().length === 0) {
            try {
              ...
              console.warn(
                "Microphone access denied for screen feed fallback.",
              );
          ...
          try {
            ...
            console.warn(
              "High-res webcam failed, trying basic audio/video:",
              e1,
            );
            try {
              mediaStream = await navigator.mediaDevices.getUserMedia({
      ...
      if (mediaStream.getAudioTracks().length > 0) {
        ...
        setErrorMessage(
          "Warning: No audio detected. Ensure 'Share system audio' is checked or microphone is active.",
        );
      }
      ...
      const sessionPromise = getAI().live.connect({
        ...
        callbacks: {
          ...
          onmessage: async (message: LiveServerMessage) => {
            ...
            const outputTranscription = (serverContent as any)
              .outputTranscription?.text;
            if (outputTranscription) {
              setCurrentTranslation((prev) => {
                const text = prev + outputTranscription;
            ...
            if (modelPart) {
              ...
              if (text) {
                ...
                if (
                  !outputTranscription ||
                  !outputTranscription.includes(text)
                ) {
                  setCurrentTranslation((prev) => {
                    const updated = prev + text;
                    currentTranslationRef.current = updated;
                    return updated;
                  });
                }
              }
            ...
            const inputTranscription = (serverContent as any).inputTranscription
              ?.text;
            if (inputTranscription) {
              setCurrentTranscription((prev) => {
                const text = prev + inputTranscription;
            ...
            if (serverContent.turnComplete) {
              ...
              if (finalTranslation || finalTranscription) {
          ...
          onerror: (err) => {
            ...
            setErrorMessage(
              "Live API Connection Error. Check your API key or network.",
            );
          },
        },
      });
  ...
  const stopRecording = () => {
    ...
    const finalTrans = currentTranscriptionRef.current;
    const finalTransl = currentTranslationRef.current;

    if (finalTrans || finalTransl) {
      setTranscriptions((prev) => [
        ...prev,
        {
          id: Date.now().toString(),
          text: finalTrans || "",
          translation: finalTransl || "",
          sender: "bot",
          timestamp: Date.now(),
        },
      ]);
  ...
  const generateScenario = async () => {
    ...
    try {
      ...
      const response = await getAI().models.generateContent({
        ...
        5. A log of any specific sales pitches made: e.g. "phone on AAL" (add a line), "new line", "port in", "tablet pitch", or "accessory".
      ...
      <header className="flex justify-between items-start">
        <div>
          <div className="flex items-center gap-2 mb-2">
            <div
              className={`w-3 h-3 rounded-full ${isRecording ? "bg-emerald-500 animate-pulse" : "bg-zinc-700"}`}
            ></div>
            <span
              className={`text-[10px] font-bold tracking-[0.2em] uppercase ${isRecording ? "text-emerald-500" : "text-zinc-500"}`}
            >
              {isRecording ? "System Live" : "System Standby"}
            </span>
          </div>
          <h1 className="text-5xl font-black tracking-tighter uppercase italic leading-none">
            AIMS SERVICES SURVEILLANCE BOT
          </h1>
        </div>
        ...
        <div className="flex flex-col items-end gap-4 text-right">
          ...
          <div className="hidden sm:block">
            <div className="text-[10px] font-mono text-zinc-500 uppercase">
              SOURCE: {isRecording ? "NEST_AUDIO_CAM_01" : "NO_SIGNAL"}
            </div>
            <div className="text-[10px] font-mono text-zinc-500 uppercase">
              LATENCY: {isRecording ? "18MS" : "--"} // G3F_SURVEILLANCE
            </div>
          </div>
      ...
      <main className="flex-1 grid grid-cols-12 gap-8 min-h-0">
        ...
        <section className="col-span-12 lg:col-span-5 flex flex-col min-h-0 gap-6">
          <div className="flex justify-end items-end">
            <div className="px-2 py-0.5 bg-zinc-800 text-[10px] font-mono rounded text-zinc-400 uppercase">
              LOG ➔ {targetLanguage.slice(0, 3)}
            </div>
          </div>
                  ...
                  {sourceType === "screen" && (
                    <div className="bg-zinc-800/50 p-4 rounded-xl border border-zinc-700 text-left space-y-2">
                      ...
                      <p className="text-[10px] text-zinc-400 leading-tight">
                        1. Click "Initialize" and select <b>Window</b>{" "}
                        (preferred) or <b>Entire Screen</b>.<br />
                        2. Choose the Google Home window from your other
                        profile.
                        <br />
                        3. <b>CRITICAL:</b> Ensure "Share system audio" is
                        checked.
                        <br />
          ...
          <div
            id="log-container"
            className="flex-1 bg-zinc-900/30 rounded-2xl p-6 border border-zinc-800 space-y-4 overflow-y-auto custom-scrollbar scroll-smooth"
          >
            ...
            <AnimatePresence mode="popLayout">
              {currentTranslation || currentTranscription ? (
                ...
                >
                  <span className="text-zinc-600 mr-2 opacity-50 font-mono text-[10px] not-italic">
                    REPORTING //
                  </span>
                  ...
                  <div className="text-emerald-400">
                    ...
                    <motion.span
                      ...
                      className="inline-block w-1 h-5 bg-emerald-500 ml-1 align-middle"
                    />
                  </div>
                </motion.div>
              ) : (
                <p className="text-zinc-600 text-xs italic font-medium">
                  Awaiting security signal...
                </p>
              )}
            </AnimatePresence>
            ...
            <div className="pt-4 space-y-4 border-t border-zinc-800/50">
              {transcriptions
                ...
                .map((t, idx) => (
                  <div
                    key={t.id}
                    className={`p-4 bg-zinc-800/20 rounded-xl border border-zinc-800/50 space-y-1 transition-all ${idx > 5 ? "opacity-30" : "opacity-100 scale-100"}`}
                  >
                    <div className="flex justify-between items-center">
                      <p className="text-[10px] font-mono text-zinc-600 tracking-tighter uppercase font-bold">
          ...
          <div className="flex justify-between items-center">
            ...
            <div className="flex gap-4">
              <div className="flex flex-col items-end">
                ...
                <span className="text-sm font-mono font-bold text-zinc-300 italic">
                  {transcriptions.length * 12} // PTS
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="flex-1 flex flex-col min-h-0">
            ...
            <div className="flex-1 bg-white text-zinc-950 rounded-3xl p-10 flex flex-col justify-between shadow-2xl relative overflow-hidden group">
              ...
              <div className="relative z-10">
                <span className="text-[10px] font-black uppercase tracking-[0.3em] bg-zinc-950 text-white px-3 py-1.5 inline-block mb-10 transition-transform group-hover:translate-x-1">
                  Synthesis Active
                </span>
              ...
              <div className="relative z-10 grid grid-cols-1 gap-6 border-t border-zinc-100 pt-10 mt-10 overflow-hidden">
                <div className="max-h-[300px] overflow-y-auto custom-scrollbar-light pr-4">
                  <p className="text-[10px] font-bold uppercase mb-4 tracking-widest text-zinc-400 font-mono">
                  ...
                  <div className="text-sm font-medium leading-relaxed uppercase tracking-tight text-zinc-800 whitespace-pre-wrap">
                    {scenario ||
                      "Awaiting sufficient conversational data to synthesize a full security assessment report. Ensure your Google Home feed is active and unmuted."}
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="mt-8 grid grid-cols-1 sm:grid-cols-3 gap-6">
              <div className="bg-zinc-900 p-6 rounded-2xl border border-zinc-800">
                <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest mb-1 font-mono">
                  Sentiment
                </p>
      ...
      <footer className="mt-4 flex flex-col sm:flex-row justify-between items-center border-t border-zinc-900/50 pt-10 gap-8">
        <div className="flex gap-16">
          <div className="flex flex-col">
            ...
            <span className="text-3xl font-black italic text-zinc-400 tracking-tighter leading-none mt-1">
